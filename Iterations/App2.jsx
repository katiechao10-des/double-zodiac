...
// Frame paths: 01.jpeg through 13.jpeg in /public/frames/
const FRAME_PATHS = Array.from({ length: 13 }, (_, i) =>
  `/frames/${String(i + 1).padStart(2, "0")}.jpeg`
);
...
style.textContent = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .envelope-frame {
    ...
    max-width: 80vw;
    border-radius: 4px;
    box-shadow: 0 8px 32px rgba(26, 20, 16, 0.15), 0 2px 8px rgba(26, 20, 16, 0.1);
    cursor: pointer;
  ...
  .envelope-frame:hover {
    box-shadow: 0 12px 40px rgba(26, 20, 16, 0.2), 0 4px 12px rgba(26, 20, 16, 0.12);
  }
  ...
  .paper-image { width: 100%; border-radius: 2px; display: block; }
  ...
  @media (max-width: 560px) {
    ...
    .envelope-frame { width: 240px; }
    .paper-container { width: 300px; }
    .paper-text-overlay { font-size: 17px; }
  }
`;
...
export default function App() {
  ...
  function handleEnvelopeClick() {
    ...
    if (phase === PHASE.ENVELOPE) {
      animateFrames(0, 4, () => setPhase(PHASE.BACK));
    } else if (phase === PHASE.BACK) {
      animateFrames(4, 10, () => setPhase(PHASE.REVEALED));
    }
  }

  function handleReset() {
    setPhase(PHASE.BIRTHDAY);
    setResult(null);
    setBirthday("");
    setCurrentFrame(0);
  }
    ...
    <>
      ...
      <div className="page">
        <div className="content">
          ...
          {phase === PHASE.BIRTHDAY && (
            <>
              <div className="header">
                <h1 className="header-title">AD<em>strology.</em></h1>
                <p className="header-sub">You have two zodiac signs. They don't agree on who you are.</p>
              </div>
          ...
          {(phase >= PHASE.ENVELOPE && phase <= PHASE.OPENING) && (
            <div className="envelope-stage">
              <img
                src={FRAME_PATHS[currentFrame]}
                alt="Red envelope"
                className={`envelope-frame${phase === PHASE.FLIPPING || phase === PHASE.OPENING ? " no-click" : ""}`}
                onClick={handleEnvelopeClick}
                draggable={false}
              />
              {phase === PHASE.ENVELOPE && <div className="envelope-hint">Tap to flip</div>}
              {phase === PHASE.BACK && <div className="envelope-hint">Tap to open</div>}
              {(phase === PHASE.FLIPPING || phase === PHASE.OPENING) && <div className="envelope-hint" style={{ animation: "none", opacity: 0.4 }}>...</div>}
            </div>
          )}
          ...
          {phase === PHASE.REVEALED && result && (
            <div className="result-stage">
              ...
              <div className="paper-container">
                <img src={FRAME_PATHS[10]} alt="Uncrumpled paper" className="paper-image" />
